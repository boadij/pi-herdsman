...
Only explicit `staff.delegate` starts or recovers delegation; roster and report reads never resume external mutations. An unresolved creation/start remains visible and blocks an unrelated delegate request rather than reusing its old task, files, or identity. Herdsman allocates an assignment and persists its branch and selected base before creating the workspace: a requested branch is retained, otherwise the branch is `herdsman/<assignment-id>`. It passes those same explicit values on every recovery. Ambiguous creation is reconciled against the persisted branch/base and Herdr topology; if a branch exists without an open workspace, Herdr `worktree open --branch ... --no-focus` recovers it. Herdsman does not create a second branch/workspace for the assignment. Herdr controls the workspace path and label. The exact pane is started only after bounded shell-readiness and ownership checks; the assignment is durably delivered after verifying the exact Lead. Manager `staff.list` includes current Leads in its worktree group (including manually created Leads) and unresolved assignment phases.
