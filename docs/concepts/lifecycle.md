...
generation for the new assignment. The continuation uses the saved cwd and
session history together with the current effective authorized definition
configuration. Use `delegate` with `fork` when a separate derived session is
required.
