...
orchestration. You do not need to type structured `worker` requests for this
path.
...
The extension distinguishes a lead Pi session, valid managed workers, and
...
While workers work, you can continue discussing or planning with the
lead Pi session, delegate other independent work, inspect active work, or focus a
managed worker pane.

You do not need to poll for completion. Results and worker questions are
...
See [`/workers` commands](reference/commands.md) for exact behavior.
...
Workers exist for one assignment only. After a terminal result is delivered,
Pi Herdsman cleans up the worker; the exact Pi session remains available for
continuation. The `Running` menu shows the complete authoritative inventory of
live or unresolved lifecycle work.

Managed worker panes show their validated breadcrumb identity. Their own active
...
request. This starts a new worker generation with the saved session and current
...
This is optional and does not change worker ownership. In a UI session, use
...
- Use [`/workers` commands](reference/commands.md) when you need human control.
