...
Only `task` is required. `staff.delegate` is the only operation that starts or recovers delegation; roster, inspect, transcript, message, and reply calls never resume it. An unresolved `creating` or `starting` assignment remains visible and blocks an unrelated delegation request; it is never silently substituted for the new task. The Manager allocates an assignment ID and chooses both branch and base before any Herdr mutation: it retains a caller-supplied `branch`, or defaults to `herdsman/<assignment-id>`, and persists the selected base. Every recovery attempt uses those same persisted values; it does not infer a new base from the current HEAD or request. It persists the assignment with its branch and base, then invokes Herdr with an explicit branch and base:

```text
herdr worktree create --workspace <primary-workspace-id> --branch <branch> --base <base> --no-focus
```

If omitted, the selected base is resolved and persisted before creation. Herdr determines the linked-worktree workspace path and label. The Manager uses the exact returned workspace, tab, and pane identities and the normal bounded shell-readiness and ownership checks before starting Pi, verifies one exact ordinary Lead, then durably delivers the assignment. On success it returns assignment ID, exact Lead session ID, workspace ID, and branch. There is no caller-selected cwd, pane, Agent definition, focus, review, or merge policy. The linked-worktree workspace remains after completion.

An uncertain external creation result is reconciled only by an explicit `staff.delegate` recovery request, using the persisted branch, base, and authoritative Herdr worktree topology. If the branch exists but has no open workspace identity, Herdr's `worktree open --branch ... --no-focus` recovers it. Do not generate another branch or create a second workspace for that assignment; do not blindly retry or delete a possible linked-worktree workspace. An unresolved create/start failure remains visible in its last known phase with a durable error. A Lead's result enters `settling`; missing result notification is requeued for the current Manager, and accepted Manager delivery removes the assignment. Pass the retained `result:<assignment-id>` to another Lead through `files`.
