...
test("root Lead explicitly enters Manager; a competing root session stays Lead", async () => {
  ...
  try {
    ...
    try {
      ...
      const peer = first.tools.find((tool) => tool.name === "peer");
      const visible = await peer.execute(
        "list",
        { action: "list" },
        undefined,
        undefined,
    ...
    assert.deepEqual(second.pi.getActiveTools(), [
      "read",
      "agent",
      "supervisor",
      "peer",
    ...
    try {
      ...
      assert.deepEqual(replacement.pi.getActiveTools(), [
        "read",
        "agent",
        "supervisor",
        "peer",
...
test("Manager delegate persists an exact worktree Lead assignment", async () => {
  setLeadEnvironment();
  process.env.HERDR_PANE_ID = "root-pane";
  process.env.HERDR_TAB_ID = "root-tab";
  process.env.HERDR_SOCKET_PATH = join(
  ...
  try {
    ...
    const outcome = await staff.execute(
      "delegate",
      { action: "delegate", task: "Implement focused change" },
      undefined,
      undefined,
      ctx,
    );
    ...
    try {
      ...
      const supervisor = lead.tools.find((tool) => tool.name === "supervisor");
      await assert.rejects(
        supervisor.execute(
          "invalid",
          { action: "result", message: "wrong" },
          undefined,
          undefined,
          leadCtx,
        ),
      ...
      const result = await supervisor.execute(
        "result",
        { action: "result", result: "Finished implementation" },
        undefined,
        undefined,
        leadCtx,
      );
...
test("a Lead without a Manager routes upward to Chief and keeps peer presence", async () => {
    ...
    [
      {
        message: {
          role: "assistant",
          content: [{ type: "toolCall", name: "supervisor" }],
        },
      },
    ],
  ) as any;
  ...
  try {
    ...
    assert.equal(stillPresent.claim.id, initial.claim.id);
  } finally {
    await pi.events.get("session_shutdown")?.[0]();
    lease.release();
    delete process.env.HERDR_SOCKET_PATH;
...
test("Chief leave restores minimal peer presence before provenance resolves", async () => {
  ...
  const pi = fakeChiefPi({
    ...
    exec: async (_command, args) => {
      if (args[0] === "workspace" && args[1] === "get") {
        ...
        if (call > 1) await releaseProvenance.promise;
        return {
          stdout: JSON.stringify({
            id: AGENT_ID,
            result: {
              workspace: {
                label: call === 2 ? "fresh-generation" : "stale-generation",
              },
            },
          }),
          stderr: "",
  ...
  try {
    ...
    assert.equal(initial.workspaceLabel, "tmp");
...
test("peer delivery survives sender shutdown and is accepted exactly once", async () => {
  ...
  try {
    ...
    const queued = await peer.execute(
      "message",
      { action: "message", session: targetId, message: "sender survived" },
      undefined,
      undefined,
      senderContext,
    );
...
test("lead session-start retries an exact baseline after restoration fails", async (t) => {
  ...
  await pi.commandOptions.get("chief").handler("", context);
  const baseline = ["read", "bash", "agent", "supervisor", "peer"];
  entries.push({
    type: "custom",
    customType: "pi-herdsman-role",
...
test("lead session-start continues when chief lease release fails", async () => {
  ...
  entries.push({
    ...
    data: {
      role: "lead",
      leadTools: ["read", "bash", "agent", "supervisor", "peer"],
    },
  });
...
test("Chief activation replaces the lead widget and overview selection is interactive", async (t) => {
  ...
  const entries: unknown[] = [];
  const pi = fakeChiefPi({
    activeTools: ["read", "agent", "supervisor"],
    autoActivateRegisteredTools: true,
    entries,
    exec: (_command, args) => {
  ...
  await pi.commandOptions.get("chief").handler("", context);
  assert.deepEqual(notices, ["Chief mode active."]);
  assert.deepEqual(pi.pi.getActiveTools(), ["staff"]);
...
test("Lead resume repairs stale staff from its durable displaced loadout", async () => {
  ...
  const entries: unknown[] = [];
  const ordinaryTools = ["read", "bash", "agent", "supervisor", "peer"];
  const pi = fakeChiefPi({
  ...
  assert.deepEqual(reloaded.pi.getActiveTools(), [
    "read",
    "bash",
    "agent",
    "supervisor",
...
test("ordinary branch tool state wins over an older lead checkpoint", async () => {
  ...
  const branchTools = ["read", "grep", "agent", "supervisor", "peer"];
...
async function openChiefOverview(
  ...
  const agents = populated
    ...
    : [];
  const pi = fakeChiefPi({
    activeTools: ["agent", "supervisor", "read"],
    entries: [],
    exec: (_command, args) => {
...
test("Chief resume rejects a persisted pending chief ask without activation", async () => {
  ...
  const pi = fakeChiefPi({ entries, activeTools: ["agent", "peer"] });
  registerExtension!(pi.pi as never);
  const context = fakeContext(entries) as any;
  context.ui.notify = () => undefined;
  await pi.events.get("session_start")![0](undefined, context);
  assert.deepEqual(pi.pi.getActiveTools(), ["agent", "supervisor", "peer"]);
  assert.equal(
    entries.some(
      (entry: any) =>
        entry.customType === "pi-herdsman-role" && entry.data.role === "chief",
...
test("persisted chief collision is suspended and has no lead authority", async () => {
  ...
  const entries = [
    {
      type: "custom",
      customType: "pi-herdsman-role",
      data: { role: "chief", leadTools: ["agent", "supervisor"] },
    },
  ];
  ...
  try {
    const pi = fakeChiefPi({ entries, activeTools: ["agent", "supervisor"] });
    registerExtension!(pi.pi as never);
    const context = fakeContext(entries) as any;
    context.ui.notify = () => undefined;
    await pi.events.get("session_start")![0](undefined, context);
